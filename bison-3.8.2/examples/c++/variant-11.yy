...
%code // *.cc
{
    ...
    namespace yy
    {
        ...
        std::ostream& operator<<(std::ostream& o, const string_uptrs& ss)
        {
            o << '{';
            const char *sep = "";
            for(const auto& s: ss)
    ...
    string_uptr make_string_uptr(Args&&... args)
    {
...
namespace yy
{
    ...
    static parser::symbol_type yylex()
    {
        ...
        auto loc = parser::location_type{nullptr, stage, stage};
        if(stage-1 == 0)
            return parser::make_TEXT(make_string_uptr("I have numbers for you."), std::move(loc));
        else if(stage-1 < max)
            return parser::make_NUMBER(stage-1, std::move(loc));
        else if(stage-1 == max)
...
{
    if(argc >= 2 && isdigit(static_cast<unsigned char>(*argv[1])))
    {
