...
private:
    ...
    void lex_escapes(std::vector<pair<int, std::string>> escapes, /* out */ vector<vector<string>>& tokens);
    void lex(const string& s, /* out */ vector<string>& tokens);
    void parse(const vector<std::vector<string>>& tokens, /* out */ string& html); 
